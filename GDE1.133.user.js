...
// @grant        none
// @updateURL    https://github.com/Gil3X/godville-duel-entity/blob/main/GDE1.133.user.js
// @downloadURL  https://github.com/Gil3X/godville-duel-entity/blob/main/GDE1.133.user.js
// ==/UserScript==
...
(function() {
    ...
    gui.innerHTML = `
        ...
        <div style="text-align:right; font-size:9px; color:#666; border-top:1px solid #333; padding-top:4px;">Gilt3x v1.133</div>
    `;
    ...
    const updateUI = () => {
        ...
        const mL = myValidRounds > 0 ? (myTotalEff / myValidRounds) : 0;
        const oL = opValidRounds > 0 ? (opTotalEff / opValidRounds) : 0;
        const mE = document.getElementById('mLuck'), oE = document.getElementById('oLuck');
        mE.innerText = mL.toFixed(2); oE.innerText = oL.toFixed(2);

        // Цветовая индикация: кто выше - зелёный, кто ниже - красный
        if (mL > oL && Math.abs(mL - oL) > 0.5) {
            mE.style.color = "#00ff00";
            oE.style.color = "#ff0000";
        }
        else if (oL > mL && Math.abs(oL - mL) > 0.5) {
    ...
    const process = () => {
        ...
        if (curMy === null || curOp === null) {
            // В логах HP нет - удача не считается
        }
            ...
            if (lastLine) {
                ...
                if (!hasBackfire && hasAnyAction) {
                    ...
                    if (turnCounter === 0) {
                        myTotalEff += myLuck;
                        myValidRounds++;
                        log(`➕ Мой: +${myLuck.toFixed(2)} (avg: ${(myTotalEff/myValidRounds).toFixed(2)})`);
                    } else {
                        opTotalEff += opLuck;
                        opValidRounds++;
                        log(`➕ Оп: +${opLuck.toFixed(2)} (avg: ${(opTotalEff/opValidRounds).toFixed(2)})`);
                    }
